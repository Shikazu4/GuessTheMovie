document.addEventListener('DOMContentLoaded', function() {
    ...
    let attempts = 0;
    let currentImage = 1; // Track the current image number
    ...
    const handleGuessSubmission = () =>  {
        ...
        if (!isValidGuess(guess)) {
            alert('Invalid guess! Please enter a valid movie name.');
            return;
        }

        // Check if the guess is correct
        // Send a POST request to the server to check the guess
        fetch('/check_guess', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            pastGuesses.push({ attempt: attempts, guess: guess, correct: data.correct });

            // Update the image and unlock the corresponding button on a wrong guess
            if (!data.correct && attempts <= buttons.length) {
                movieImage.src = data.next_image;
                buttons[attempts].removeAttribute('disabled');
            }

            // Automatically load the next image after every guess
            ...
            if (data.correct) {
                ...
                alert('Sorry! You ran out of attempts. The correct movie is ABC.');
            }
        });
    ...
    // Handle button clicks to toggle between images
